...
import { motion } from "framer-motion" // Añadí esta línea

// 💎 Logo orgánico agrandado idéntico al Register
...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex bg-background">
      {/* 🚀 PANEL LATERAL IZQUIERDO: Estructura idéntica al Register (p-12 y justify-between) */}
      <aside 
        aria-hidden="true" 
        className="hidden lg:flex lg:w-[46%] xl:w-[44%] relative overflow-hidden flex-col justify-between p-12" 
        style={{ backgroundColor: "oklch(0.25 0.02 30)" }}
      >
        ...
        {/* 🎯 Slogan del Login con los estilos de espaciado del Register (space-y-10) */}
        <div className="relative space-y-10">
          <div className="space-y-3">
            <span className="inline-block text-[11px] font-semibold uppercase tracking-[0.18em] px-2.5 py-1 rounded-full" style={{ backgroundColor: "oklch(0.65 0.15 25 / 0.18)", color: "oklch(0.65 0.15 25)" }}>
              Tu nueva rutina empieza hoy
            </span>
            <h2 className="text-4xl xl:text-[2.6rem] font-light leading-[1.12] text-balance" style={{ color: "oklch(0.97 0.005 85)" }}>
              Construye hábitos que realmente duran.
            </h2>
          </div>
          <ul className="space-y-4">
            {PANEL_FEATURES.map((feat) => (
              <li key={feat} className="flex items-center gap-3">
                ...
                <span className="text-sm leading-snug" style={{ color: "oklch(0.97 0.005 85 / 0.6)" }}>
                  {feat}
                </span>
              </li>
            ))}
        ...
        {/* Footer del panel lateral (Tags inferiores idénticos al Register) */}
            ...
            <form className="space-y-5" onSubmit={handleSubmit} noValidate>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="#" className="text-xs text-accent hover:opacity-80 transition-opacity font-medium">
                    ¿La olvidaste?
                  </Link>
                </div>
